...
class _EventProposalsState extends State<EventProposals> {
  ...
  void _confirmSetStatus(String docId, String title, String newStatus) async {
    ...
    showDialog(
      ...
      builder: (_) => AppConfirmationDialog(
        ...
        accentColor: s.btnColor,
        icon: s.icon,
        onConfirm: () => _setStatus(docId, title, newStatus),
      ),
    );
  }
  ...
  void _showProposalDetailDialog(String docId, Map<String, dynamic> data) {
    ...
    // proportions instead of forcing every banner into a fixed strip —
    // mirrors the org-side "View Proposal" modal's image sizing.
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── BODY ────────────────────────────────────────────────
              // Fixed-height Row instead of one scrolling Column — banner
              // on the left (sized to the photo's own aspect ratio so it
              // isn't cropped into a cramped 200px strip), details
              // scrolling independently on the right.
              Flexible(
                child: SizedBox(
                  height: 560,
                  child: Row(
